...
void UART_Init(UART_Type *psUart, int port, int mode, int iBaud)
{
    ...
    if (psUart == UART1)
    {
        ...
        if (port == UART1_PORT_P14_P15)
        {
            ...
            IOM->AF0 |= IOM_AF0_P14_SEL_UART1_RX;
            IOM->AF0 &= ~IOM_AF0_P15_SEL;
            IOM->AF0 |= IOM_AF0_P15_SEL_UART1_TX;
        }
        else
        {
        	IOM->PU |= GPIO_PIN18;
            IOM->AF1 &= ~IOM_AF1_P18_SEL;
            IOM->AF1 |= IOM_AF1_P18_SEL_UART1_RX;
            IOM->AF1 &= ~IOM_AF1_P19_SEL;
            IOM->AF1 |= IOM_AF1_P19_SEL_UART1_TX;
        }
    ...
    else if (psUart == UART2)
    {
        ...
        if (port == UART2_PORT_P04_P05)
        {
            ...
            IOM->AF0 |= IOM_AF0_P04_SEL_UART2_RX;
            IOM->AF0 &= ~IOM_AF0_P05_SEL;
            IOM->AF0 |= IOM_AF0_P05_SEL_UART2_TX;
        }
        else
        {
        	IOM->PU |= GPIO_PIN6;
            IOM->AF0 &= ~IOM_AF0_P06_SEL;
            IOM->AF0 |= IOM_AF0_P06_SEL_UART2_RX;
            IOM->AF0 &= ~IOM_AF0_P07_SEL;
            IOM->AF0 |= IOM_AF0_P07_SEL_UART2_TX;
        }
